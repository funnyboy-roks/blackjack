...
<CardComp.Root class="w-full max-w-sm">
    <CardComp.Header>
        ...
        {#if !isDealer || turn}
            <CardComp.Action>
                Value: {handValue.softValue === null ? handValue.value : `${handValue.value} or ${handValue.softValue}`}
            </CardComp.Action>
        {/if}
    </CardComp.Header>
    ...
    {#if player.hasBusted()}
        <CardComp.Footer class="flex flex-col items-start gap-2">
            <h2 class="font-bold text-destructive text-2xl">
                Bust!
            </h2>
        </CardComp.Footer>
    {:else if win === 'blackjack'}
        <CardComp.Footer class="flex flex-col items-start gap-2">
            <h2 class="font-bold text-green-400 text-2xl">
                Blackjack!
            </h2>
        </CardComp.Footer>
    {:else if win === 'win'}
    ...
    {:else if win === undefined && !isDealer && turn}
        <CardComp.Footer class="flex flex-col items-start gap-2">
            ...
            <div class="flex flex-row gap-1">
                ...
                <Button disabled={!turn} onclick={() => onaction?.('double-down')}>Double Down</Button>
            </div>
        </CardComp.Footer>
    {/if}
</CardComp.Root>
